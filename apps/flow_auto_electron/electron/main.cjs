...
ipcMain.handle('dance:generate', async(_e,payload={})=>{
  ...
  const duration=videoDurationSec(payload.video)||8, segmentDuration=Math.max(4,Math.min(10,Number(payload.segmentDuration)||8)), frameCount=Math.min(40,Math.max(4,Math.ceil(duration/2)));
