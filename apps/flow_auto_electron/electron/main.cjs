...
async function generateScriptJs(payload){
  ...
  const silentSpeech=String(payload.speechMode||'fixed')==='silent';
  const batchSize=20;
  ...
  for(let i=0;i<batches;i++){
    ...
    const sys=` Bạn là một chuyên gia biên kịch và đạo diễn hình ảnh chuyên nghiệp.
      ...
      6. CHỐNG TRÙNG LẶP CẢNH: Mỗi cảnh phải có nội dung riêng theo đúng tiến trình kịch bản. Không được lặp lại cùng hành động, cùng mô tả, cùng góc máy, cùng bối cảnh hoặc cùng prompt giữa các cảnh. Cảnh sau phải phát triển trực tiếp từ kết quả/hành động cuối của cảnh trước; tư thế cuối, hướng chuyển động, vị trí chủ thể, trạng thái cảm xúc và logic thời gian phải nối sang cảnh kế tiếp. Cấm các cảnh rời rạc như những đoạn độc lập.
    ...
    const parts=[...(i===0?imgs:[]),{text:`Topic/content: ${payload.topic}. Total video scenes: ${totalScenes}. Generate scenes ${startScene}-${endScene}. ${characterInstruction} Prompts and descriptions must be in ${outLang}. ${finalPromptLanguageRule(outLang)} If ${outLang} is Vietnamese, every sentence in description and prompt must be Vietnamese; do not output English style phrases, English camera instructions, or English safety rules except unavoidable proper names. ${silentSpeech?'No character speaks: no dialogue, no voiceover, no singing, and no lip-sync speech.':`If any dialogue/speech exists, the only allowed speaker voice is ${speakerGender} using ${voiceLang};`} both speaker gender and accent must stay identical in every generated prompt. Do not mix accents/languages. Write rich, production-ready prompts rather than short summaries. Preserve character consistency and explicit continuity from the previous scene's ending state into the next scene's opening state. Each scene must be unique, must follow the exact script progression, and must not repeat the same action, camera, setting, or wording from another scene. Each scene must be detailed enough to render and must contain these fields: visual, action, emotion, cameraLighting, voice, prompt. Apply this subtitle rule to every scene: ${subtitlePromptRule(payload,outLang,voiceLang)} Apply this safety rule to every scene: ${policySafeInstruction(outLang)}`}];
    ...
    const scenes=[];
    for(const sc of (obj.scenes||[])){if(silentSpeech){sc.voice='Không có';sc.dialogue='';sc.loiThoai='';sc.prompt=String(sc.prompt||'')+' Nhân vật không nói, không lời thoại, không Voiceover, không hát và không khẩu hình nói.';}
      scenes.push({
        ...sc,
...
function collectRunnerDiagnostics(dir){
  const clean=s=>String(s||'').replace(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g,'[email]').replace(/(?:token|authorization|cookie|password|access[_ -]?key)\s*[:=]\s*[^\s]+/ig,'[redacted]');
  try{
    const logs=fs.readdirSync(DEBUG_DIR).filter(x=>/^electron-runner-.*\.log$/.test(x)).map(x=>({x,p:path.join(DEBUG_DIR,x),t:fs.statSync(path.join(DEBUG_DIR,x)).mtimeMs})).sort((a,b)=>b.t-a.t).slice(0,4);
    for(const f of logs)fs.writeFileSync(path.join(dir,`runner-${f.x}`),clean(fs.readFileSync(f.p,'utf8')).slice(-200000));
    const states=fs.readdirSync(JOB_DIR).filter(x=>/^electron-runner-state-.*\.json$/.test(x)).map(x=>({x,p:path.join(JOB_DIR,x),t:fs.statSync(path.join(JOB_DIR,x)).mtimeMs})).sort((a,b)=>b.t-a.t).slice(0,4);
    for(const f of states){try{const o=JSON.parse(fs.readFileSync(f.p,'utf8'));const safe={done:o.done,total:o.total,status:o.status,error:o.error,current:o.current,updatedAt:o.updatedAt};fs.writeFileSync(path.join(dir,`runner-${f.x}`),JSON.stringify(safe,null,2))}catch{}}
  }catch(e){fs.appendFileSync(path.join(dir,'recorder-errors.log'),'collect_runner_diagnostics:'+String(e.message||e)+'\n')}
}
async function exportFlowDebug(){await stopFlowDebug();if(!flowDebug.dir||!fs.existsSync(flowDebug.dir))return {ok:false,error:'flow_debug_recording_missing'};collectRunnerDiagnostics(flowDebug.dir);const r=await dialog.showSaveDialog({title:'Xuất gói Debug Flow',defaultPath:`flow-interaction-debug-${Date.now()}.zip`,filters:[{name:'ZIP',extensions:['zip']}]});if(r.canceled||!r.filePath)return {ok:false,canceled:true};if(process.platform==='win32'){const q=x=>`'${String(x).replace(/'/g,"''")}'`;const z=spawnSync('powershell.exe',['-NoProfile','-Command',`Compress-Archive -Path ${q(path.join(flowDebug.dir,'*'))} -DestinationPath ${q(r.filePath)} -Force`],{encoding:'utf8',windowsHide:true});if(z.status!==0)return {ok:false,error:'flow_debug_zip_failed:'+z.stderr}}else{const z=spawnSync('zip',['-qr',r.filePath,'.'],{cwd:flowDebug.dir,encoding:'utf8'});if(z.status!==0)return {ok:false,error:'flow_debug_zip_failed:'+z.stderr}}return {ok:true,file:r.filePath,count:flowDebug.events.length};}
...
function startRunner(payload){
  ...
  // Old Flow rule: an explicitly selected image folder is the source of truth.
  // Character images are only converted into a temp refs folder when no refsDir was chosen.
  const selectedRefsDir=String(payload.refsDir||'').trim();
  const characterImages=selectedRefsDir ? [] : (payload.characterImages||[]);
  ...
  threadFiles.forEach((pf,idx)=>{
    ...
    args.push(payload.pairedMode===false?'--no-paired-mode':'--paired-mode'); args.push('--ref-mode',String(payload.refMode||'paired')); if(payload.allowMultiRefs===true) args.push('--allow-multi-refs'); const wantAutoDownload = payload.autoDownload !== false; if(wantAutoDownload) args.push('--auto-download'); if(wantAutoDownload && payload.runMode==='continuous_submit_only') args.push('--download-delay-prompts','3'); if(!wantAutoDownload && payload.runMode==='continuous_submit_only') args.push('--submit-only'); const refDir=threadRefs[idx]||payload.refsDir; if(refDir) args.push('--refs-dir',refDir); if(payload.downloadDir) args.push('--output-dir',payload.downloadDir); try{ fs.appendFileSync(logFile, `[runner] path=${runner.path||runner.cmd} compiled=${!!runner.compiled}\n[runner] thread=${idx+1} mode=${payload.mode||payload.taskMode} model=${payload.model||payload.flowModel} ratio=${payload.ratio||payload.aspectRatio||payload.flowAspectRatio} count=${payload.count||payload.flowCount} autoDownload=${wantAutoDownload} runMode=${payload.runMode||''} refsDir=${payload.refsDir||''} characterImages=${characterImages.length}\n`); }catch{}
...
ipcMain.handle('seo:analyze', async(_e,payload={})=>{
  ...
  try{
    ...
    const system='You are a YouTube and TikTok SEO strategist. Analyze only observable video content. Return strict JSON with youtubeTitle (accurate, high-CTR, maximum 100 characters), tiktokTitle, description (search-first and factual), hashtags (15-20 relevant strings), keywords (relevant broad and long-tail strings with comma-joined total at most 500 characters), viralScore (integer 0-100 representing potential, never a guarantee), hook (first-three-second hook), and optimalPostTimes (7 objects with day and 2-3 suggested local times). Avoid unrelated trends, keyword stuffing, fabricated facts, and guaranteed-result claims.';
...
ipcMain.handle('dance:generate', async(_e,payload={})=>{
  ...
  const wardrobeOnly=payload.generationMode==='wardrobe';
  if(!wardrobeOnly&&(!payload.video||!fs.existsSync(payload.video)))return {ok:false,error:'missing_dance_video'};
  ...
  const parts=[{text:wardrobeOnly?'WARDROBE-ONLY MODE. Ignore dance, choreography, motion, video and timeline. Analyze the model image and create one still-image wardrobe replacement prompt per outfit set.':`Authorized media only.`},imgPart(payload.modelImage)];
  let duration=0,segmentDuration=8;
  if(!wardrobeOnly){
    duration=videoDurationSec(payload.video)||8; segmentDuration=Math.max(4,Math.min(10,Number(payload.segmentDuration)||8));
    const pattern=path.join(work,'dance_%03d.jpg'),frameCount=Math.min(40,Math.max(4,Math.ceil(duration/2)));
...
ipcMain.handle('prompt:rewriteScriptDeep', async(_e,payload={})=>{
  ...
  if(!apiKey)return {ok:false,error:'missing_api_key'}; if(!source)return {ok:false,error:'missing_script'};
  const outLang=langName(payload.promptLang), duration=String(payload.duration||'60 seconds'), targetScenes=durationScenes(duration);
  const voice=voiceLangName(payload.voiceLang), gender=String(payload.speakerGender||'male')==='female'?'nữ':'nam';
  const sys=`Bạn là biên kịch video chuyên sâu và đạo diễn hình ảnh. Viết lại kịch bản người dùng bằng ${outLang}, chi tiết, giàu hình ảnh, logic và cuốn hút hơn nhưng giữ nguyên tuyệt đối dữ kiện, nhân vật, tên riêng, số liệu, phát ngôn, trình tự sự kiện và thông điệp cốt lõi; không bịa thêm. Tạo đúng ${targetScenes} cảnh, mỗi cảnh được thiết kế để sử dụng gần đủ 8 giây và phải phát triển một ý cụ thể trong tiến trình gốc. YÊU CẦU ĐỘ CHI TIẾT CHO TỪNG CẢNH: Hình ảnh gồm 2–4 câu mô tả rõ chủ thể, bối cảnh, tiền cảnh/hậu cảnh và chi tiết thị giác quan trọng; Hành động gồm 2–3 bước chuyển động liên tiếp thay vì một động từ ngắn; Cảm xúc mô tả trạng thái, sắc thái và nhịp cảm xúc; Camera nêu cỡ cảnh, góc máy, hướng/chuyển động máy và điểm lấy nét; Ánh sáng/Màu sắc nêu nguồn sáng, độ tương phản, bảng màu và không khí; Voiceover phải tự nhiên, có mở–phát triển–chốt ý, giàu nhịp điệu, dùng từ gợi hình nhưng chính xác, truyền tải trọn ý cảnh và đủ đọc trong khoảng 7–8 giây, không chỉ là một cụm từ hoặc câu sơ sài. Mở đầu có hook mạnh dựa trên nội dung gốc; các cảnh sau phát triển theo đúng bối cảnh, diễn biến, nguyên nhân, tác động, góc nhìn và kết luận đã có. Loại bỏ lặp ý, quảng cáo và câu thừa. Voiceover toàn kịch bản dùng duy nhất giọng ${gender}, ${voice}, không đổi chất giọng giữa các cảnh. Trả JSON {"script":"..."}. Mỗi cảnh chỉ gồm Scene NN, Hình ảnh, Hành động, Cảm xúc, Camera, Ánh sáng/Màu sắc, Voiceover. Không thêm Prompt, Description hoặc Thời lượng.`;
  try{
    const requestText=`KỊCH BẢN GỐC:\n${source}\n\nHãy viết lại chuyên sâu, giữ đúng dữ kiện và đúng ${targetScenes} cảnh. Mỗi trường của mỗi cảnh phải đầy đủ, cụ thể và có chiều sâu; không trả mô tả một câu ngắn. Voiceover phải hay, tự nhiên, nối mạch giữa các cảnh và đủ nội dung cho gần trọn 8 giây.`;
...
ipcMain.handle('prompt:analyzeUrl' , async(_e,payload={})=>{
  ...
  const sys=`Bạn là đạo diễn sáng tạo, biên kịch quảng cáo/video ngắn và chuyên gia phân tích nội dung. Ngôn ngữ đầu ra: ${outLang}. Nhiệm vụ: phân tích nguồn thật kỹ rồi viết lại thành kịch bản video chuyên nghiệp, cuốn hút hơn, nhưng vẫn bám sát nguồn. ${deepRewrite?'CHẾ ĐỘ VIẾT LẠI CHUYÊN SÂU TỪ BÀI BÁO — SOURCE-LOCK NGHIÊM NGẶT: toàn bộ kịch bản chỉ được diễn đạt lại thông tin xuất hiện trực tiếp trong SOURCE TEXT. Giữ nguyên chủ đề trung tâm, nhân vật, tổ chức, địa điểm, mốc thời gian, số liệu, trích dẫn, quan hệ nguyên nhân–kết quả, mức độ chắc chắn và thứ tự diễn biến của bài báo. Không bổ sung kiến thức nền từ trí nhớ, không suy đoán động cơ/hậu quả, không tạo góc nhìn mới, không thêm ví dụ, không thay chủ thể, không đổi sự kiện và không viết sang chủ đề liên quan khác. Nếu nguồn không nói thì tuyệt đối không đưa vào. Chuyên sâu chỉ bằng cách tổ chức rõ hơn, giải thích lại đúng các ý đã có và loại bỏ quảng cáo/nội dung lặp; không chuyên sâu bằng dữ kiện mới. Mọi câu Voiceover phải truy nguyên được về SOURCE TEXT. Với mỗi cảnh, Hình ảnh phải có 2–4 câu mô tả rõ chủ thể, bối cảnh, tiền cảnh/hậu cảnh và chi tiết thật có thể suy ra trực tiếp từ nguồn; Hành động phải trình bày 2–3 bước diễn biến liên tiếp từ nội dung bài; Cảm xúc phải nêu sắc thái và nhịp cảm xúc phù hợp thông tin nguồn; Camera phải có cỡ cảnh, góc máy, chuyển động và điểm lấy nét; Ánh sáng/Màu sắc phải nêu nguồn sáng, tương phản, bảng màu và không khí; Voiceover phải có câu dẫn, phát triển và điểm nhấn kết cảnh, giàu nhịp điệu, truyền tải trọn một ý có trong bài và đủ đọc khoảng 7–8 giây. Không được dùng yêu cầu chi tiết này làm lý do để bịa chi tiết không có trong SOURCE TEXT; nếu nguồn thiếu chi tiết hình ảnh thì dùng cách thể hiện trung tính như đồ họa dữ liệu, bản đồ, tiêu đề báo hoặc hình minh họa không khẳng định dữ kiện mới.':'CHẾ ĐỘ PHÂN TÍCH TIÊU CHUẨN.'} Nếu nguồn là VIDEO và có frame ảnh, hãy phân tích từng cảnh: nhân vật/chủ thể, bối cảnh, hành động, nhịp dựng, cảm xúc, camera, ánh sáng, màu sắc, điểm nhấn thị giác. Nếu nguồn là BÀI BÁO/TRANG WEB, hãy bóc tách ý chính, sự kiện, nhân vật/chủ thể, bối cảnh, thông tin quan trọng rồi chuyển thành kịch bản video. BẮT BUỘC giữ đúng sự kiện/chủ thể/thứ tự ý cốt lõi, không bịa thông tin không có trong nguồn. BẮT BUỘC chia đúng ${targetScenes} cảnh, mỗi cảnh 8 giây, không thiếu không thừa. ${policySafeInstruction(outLang)} Trả JSON {"script":"...","sourceSummary":"...","sceneAnalysis":"..."}. Trường script phải có đúng ${targetScenes} cảnh. Mỗi cảnh chỉ dùng đúng cấu trúc sau, không thêm dòng Prompt, không thêm Description, không thêm Thời lượng:
Scene 01:
Hình ảnh: ...
Hành động: ...
Cảm xúc: ...
...
Ánh sáng/Màu sắc: ...
Voiceover: ...`;
  const prompt=`URL: ${page.url}\nContent-Type: ${page.contentType}\nTitle: ${page.title||''}\nTarget duration: ${duration}\nRequired scenes: ${targetScenes} scenes, each scene 8 seconds.\nVideo analysis note: ${videoNote}\n\nSOURCE TEXT / METADATA:\n${page.text}\n\n${deepRewrite?'Hãy viết lại bài báo thành kịch bản video chuyên sâu, hấp dẫn và có chiều sâu phân tích; mở đầu bằng hook, phát triển theo bối cảnh–diễn biến–tác động–góc nhìn–kết luận, nhưng CHỈ diễn đạt lại nội dung có nguyên văn hoặc ý tương đương trong SOURCE TEXT; không dùng kiến thức ngoài nguồn và không đổi chủ đề.':'Hãy phân tích nguồn thật chi tiết rồi viết lại thành kịch bản chuyên nghiệp dùng trực tiếp trong AI Prompt Studio.'} Kịch bản cuối BẮT BUỘC đúng ${targetScenes} cảnh, mỗi cảnh 8 giây, đánh số Scene 01 đến Scene ${String(targetScenes).padStart(2,'0')}. Mỗi cảnh chỉ gồm đúng các dòng: Hình ảnh, Hành động, Cảm xúc, Camera, Ánh sáng/Màu sắc, Voiceover. Không thêm dòng Prompt, không thêm Description, không thêm Thời lượng. Không được trả bất kỳ trường nào dưới dạng một cụm từ ngắn; mỗi trường phải cụ thể, chi tiết và phục vụ trực tiếp cho ý của cảnh.`;
  try{
