...
function App(){
  ...
  return <div className="app">{bootLoading&&<div className="boot-loading"><div className="loader-card"><div className="spinner"></div><b>Đang tải ứng dụng... {bootPct}%</b><div className="boot-bar"><div style={{width:`${bootPct}%`}}></div></div><span>FLOW AUTO VEO 3 đang khởi động, vui lòng chờ.</span></div></div>}{langNotice&&<div className="modal-backdrop"><div className="small-modal"><b>{T('Đã đổi ngôn ngữ','Language changed')}</b><p>{T('Vui lòng khởi động lại app để áp dụng đầy đủ cài đặt ngôn ngữ.','Please restart the app to fully apply the language setting.')}</p><Button variant="primary" onClick={()=>setLangNotice(false)}>OK</Button></div></div>}
    ...
    <main className="main">
      ...
      {page==='outfit' && <div className="grid">
        <Card title="Thay quần áo model" icon={<Scissors/>}>
            <div className="actions"><Button onClick={pickImages}>🖼 Chọn ảnh model mẫu</Button><Button onClick={pickRefs}>📁 Chọn thư mục quần áo</Button><Button onClick={pickSampleVideo}>🎞 Chọn video mẫu nhảy</Button></div>
            <p className="hint">Model: {characterImages[0]||'chưa chọn'} • Quần áo: {refsDir||'chưa chọn'} • Video nhảy: {sampleVideo||'chưa chọn'}</p>
            <div className="form4"><Field label="Chế độ"><select value={mode} onChange={e=>setMode(e.target.value)}><option value="createimage">Tạo ảnh</option><option value="createvideo">Tạo video</option></select></Field><Field label="Model"><select value={model} onChange={e=>{setModel(e.target.value);localStorage.setItem('flow_model',e.target.value)}}>{availableModels.map(x=><option key={x} value={x}>{x}</option>)}</select></Field></div>
            <div className="actions"><Button variant="primary" onClick={async()=>{append('🚀 Đang chạy thay đồ...'); append(await api().startOutfit({characterImage:characterImages[0], refsDir, sampleVideo, mode, model}))}}>▶ Chạy thay quần áo</Button><Button variant="danger" onClick={stop}>⏹ Stop</Button></div>
        </Card>
      </div>}
</main>
  </div>
