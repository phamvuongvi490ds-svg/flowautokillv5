    ...
    <main className="main">
      ...
      {page==='multi'&&<div className="multi-page">
        <Card title="Đa luồng profile Flow" icon={<Film/>}>
          ...
          <div className="form4"><Field label="Số profile"><select value={String(profiles.length)} onChange={e=>{const n=Number(e.target.value); setProfiles(p=>Array.from({length:n},(_,i)=>p[i]||{name:`Profile ${i+1}`,script:''}))}}>{Array.from({length:100},(_,i)=>String(i+1)).map(x=><option key={x} value={x}>{x} profile</option>)}</select></Field><Field label="Model"><select id="flow-model-multi" value={model} onChange={e=>setModel(e.target.value)}>{models.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Tỉ lệ"><select id="flow-ratio-multi" value={ratio} onChange={e=>setRatio(e.target.value)}>{ratios.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>setSpacing(e.target.value)}/></Field></div>
