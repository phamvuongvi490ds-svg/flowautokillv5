...
const api = () => window.flowAPI || {
  ...
  videoList: async()=>({ok:true,files:[]}), videoMerge: async()=>({ok:false,error:'flowAPI_not_ready'}), videoExtractAudio: async()=>({ok:false,error:'flowAPI_not_ready'}), videoAnalyzeSample: async()=>({ok:false,error:'flowAPI_not_ready'}), videoPostPlan: async()=>({ok:false,error:'flowAPI_not_ready'}), videoPostExport: async()=>({ok:false,error:'flowAPI_not_ready'}), danceGenerate: async()=>({ok:false,error:'flowAPI_not_ready'}), danceExtractAudio: async()=>({ok:false,error:'flowAPI_not_ready'}), seoAnalyze: async()=>({ok:false,error:'flowAPI_not_ready'}), seoThumbnail: async()=>({ok:false,error:'flowAPI_not_ready'})
};
...
function App(){
  ...
  async function pickSeoVideo(){const r=await api().openFile({properties:['openFile'],filters:[{name:'Video',extensions:['mp4','mov','mkv','webm','avi','m4v']}]});if(r?.[0]){setSeoVideo(r[0]);setSeoResult(null);setSeoThumbnail('');append(`Video SEO: ${r[0]}`)}}
  async function analyzeSeoVideo(){if(!seoVideo){append('❌ Hãy chọn video cần phân tích SEO.');return}if(!firstKey()){append('❌ Hãy nhập Gemini API key trong AI Prompt Studio.');return}setSeoBusy(true);setSeoThumbnail('');append('AI đang phân tích video và tạo bộ SEO...');try{const r=await api().seoAnalyze({file:seoVideo,apiKey:apiKeys,apiModel:geminiApiModel,language:seoLanguage});if(r?.result)setSeoResult(r.result);append(r)}finally{setSeoBusy(false)}}
  async function generateSeoThumbnail(){if(!seoResult?.thumbnailPrompt)return;setSeoBusy(true);append('AI đang tạo thumbnail 16:9...');try{const r=await api().seoThumbnail({apiKey:apiKeys,prompt:seoResult.thumbnailPrompt});if(r?.dataUrl)setSeoThumbnail(r.dataUrl);append(r)}finally{setSeoBusy(false)}}
  ...
  async function generateScript(){
    ...
    try {
      const r=await api().generateScript({apiKey:apiKeys,apiModel:geminiApiModel,style,topic:scriptTopic,duration,characterImages,refsDir,promptLang,voiceLang,speakerGender,speechMode,promptSubtitles});
      if(r?.ok) {
        if(r.generated?.file) setGeneratedFile(r.generated.file);
