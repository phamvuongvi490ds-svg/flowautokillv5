...
<Field label="Chế độ chạy"><select id="flow-run-mode" value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 1 prompt</option></select></Field><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là tải từng prompt; chế độ liên tục sẽ tải giãn cách 1 prompt để đủ ảnh/video hơn.</small></Field><Field label="Số tab Flow"><select id="flow-threads" value={flowThreads} onChange={e=>setFlowThreads(e.target.value)}>{Array.from({length:100},(_,i)=>String(i+1)).map(x=><option key={x} value={x}>{x} tab</option>)}</select></Field></div></Card><Card title="Điều khiển" icon={<Play/>}><div className="actions"><Button variant="primary" onClick={()=>start()}><Play size={16}/> Bắt đầu</Button><Button variant="danger" onClick={stop}><Square size={16}/> Stop</Button></div></Card></div>}
...
<Field label="Chế độ chạy"><select value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 1 prompt</option></select></Field><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là tải từng prompt; chế độ liên tục sẽ tải giãn cách 1 prompt để đủ ảnh/video hơn.</small></Field></div>
...
<Field label="Chế độ chạy"><select value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 1 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>{setSpacing(e.target.value);localStorage.setItem('flow_spacing',e.target.value)}}/></Field>
          </div>
          <div className="actions"><Button onClick={saveFlowSettings}>💾 Lưu cấu hình setting</Button></div><div className="form4"><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là tải từng prompt; chế độ liên tục sẽ tải giãn cách 1 prompt để đủ ảnh/video hơn.</small></Field></div><div className="actions"><Button variant="primary" onClick={()=>start(generatedFile,{autoDownload,refsDir,pairedMode:false})}>▶ Chạy prompt AI</Button><Button variant="danger" onClick={stop}>⏹ Stop</Button></div>
        </Card>
      </div>}
...
<Field label="Chế độ chạy"><select id="flow-run-mode-multi" value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 1 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>{setSpacing(e.target.value);localStorage.setItem('flow_spacing',e.target.value)}}/></Field></div>
