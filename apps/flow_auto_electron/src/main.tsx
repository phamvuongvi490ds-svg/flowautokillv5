...
const api = () => window.flowAPI || {
  openFile: async()=>[], status: async()=>({ok:true,running:false}), outputTimeline: async()=>({ok:true,items:[]}), licenseCached: async()=>({ok:true}), machineId: async()=>({machineId:''}),
...
function App(){
  ...
  const [danceDownloadDir,setDanceDownloadDir]=useState(localStorage.getItem('dance_download_dir')||'');
  ...
  const nav=[['flow',T('Vận hành Flow','Flow Operation'),Film],['ai','AI Prompt Studio',Wand2],['chars','Prompt nhân vật',ImagePlus],['multi',T('Đa luồng','Multi-profile'),Film],['post',T('Hậu kì video','Video Post-production'),Scissors],['dance','Dance Prompt Studio',Music],['status',T('Trạng thái chạy','Run Status'),Play],['payment',T('Thanh toán','Payment'),CreditCard],['license','License',KeyRound]];
  ...
  async function pickDanceDownloadDir(){const r=await api().openFile({properties:['openDirectory']});if(r?.[0]){setDanceDownloadDir(r[0]);localStorage.setItem('dance_download_dir',r[0])}}
  ...
  useEffect(()=>{
    ...
    let active=true;
    const refresh=async()=>{try{const r=await api().outputTimeline([danceDownloadDir,downloadDir]);if(active&&r)setOutputTimeline(r)}catch{}};
    refresh();const it=setInterval(refresh,2000);return()=>{active=false;clearInterval(it)};
  },[page,danceDownloadDir,downloadDir]);
