...
function App(){
  ...
  async function checkLicense(){ const r=await api().licenseCheck(); const msg=friendly(r); setLicenseText(msg); append(msg); const ok=!!r?.ok && !String(r?.reason||r?.error||'').match(/expired|revoked|invalid|missing/i); setLicenseLocked(!ok); setLicenseReady(true); if(ok)setPage('flow'); return r }
