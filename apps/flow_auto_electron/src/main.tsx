...
function App(){
  ...
  async function exportPost(){append('Đang xuất hậu kì video AI tự động...'); let subtitles:any[]=[]; try{subtitles=JSON.parse(manualSubs||'[]')}catch{} append(await api().videoPostExport({folder:videoFolder,files:videoFiles,scenes:timeline.length?timeline:videoFiles.map((f,i)=>({file:f,keep:true,order:i+1})),subtitles,autoSub,script:postScript,srtFile,subStyle:{font:subFont||'Arial',color:subColor,size:Number(subSize)||42},musicFile,subLang,defaultTransition}))}
  const transitionOptions=[['ai','AI tự chọn'],['fade','Mờ dần'],['fadeblack','Mờ qua màu đen'],['fadewhite','Mờ qua màu trắng'],['smoothleft','Trượt mượt sang trái'],['smoothright','Trượt mượt sang phải'],['smoothup','Trượt mượt lên trên'],['smoothdown','Trượt mượt xuống dưới']];
  ...
  async function generateScript(){
    ...
    try {
      const r=await api().generateScript({apiKey:apiKeys,apiModel:geminiApiModel,style,topic:scriptTopic,duration,characterImages,refsDir,promptLang,voiceLang,promptSubtitles});
      if(r?.ok) {
        if(r.generated?.file) setGeneratedFile(r.generated.file);
...
<Field label="Chế độ chạy"><select id="flow-run-mode" value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 2 prompt</option></select></Field><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là tải từng prompt; chế độ liên tục sẽ tải giãn cách 2 prompt để đủ ảnh/video hơn.</small></Field><Field label="Số tab Flow"><select id="flow-threads" value={flowThreads} onChange={e=>setFlowThreads(e.target.value)}>{Array.from({length:100},(_,i)=>String(i+1)).map(x=><option key={x} value={x}>{x} tab</option>)}</select></Field></div></Card><Card title="Điều khiển" icon={<Play/>}><div className="actions"><Button variant="primary" onClick={()=>start()}><Play size={16}/> Bắt đầu</Button><Button variant="danger" onClick={stop}><Square size={16}/> Stop</Button></div></Card></div>}
...
<Field label="Chế độ chạy"><select value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 2 prompt</option></select></Field><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là tải từng prompt; chế độ liên tục sẽ tải giãn cách 2 prompt để đủ ảnh/video hơn.</small></Field></div>
      ...
      {page==='ai'&&<div className="grid ai">
        <Card title="API & Kịch bản" icon={<Wand2/>}>
          ...
          <div className="actions">
            ...
            <Button onClick={analyzeUrlForAi}>🌐 Phân tích link thành kịch bản</Button><Button variant="primary" onClick={rewriteArticleDeepForAi}>📰 Viết lại chuyên sâu từ link báo</Button>
...
<Field label="Chế độ chạy"><select value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 2 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>{setSpacing(e.target.value);localStorage.setItem('flow_spacing',e.target.value)}}/></Field>
          </div>
          <div className="actions"><Button onClick={saveFlowSettings}>💾 Lưu cấu hình setting</Button></div><div className="form4"><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là tải từng prompt; chế độ liên tục sẽ tải giãn cách 2 prompt để đủ ảnh/video hơn.</small></Field></div><div className="actions"><Button variant="primary" onClick={()=>start(generatedFile,{autoDownload,refsDir,pairedMode:false})}>▶ Chạy prompt AI</Button><Button variant="danger" onClick={stop}>⏹ Stop</Button></div>
        </Card>
      </div>}
...
<Field label="Chế độ chạy"><select id="flow-run-mode-multi" value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải giãn cách 2 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>{setSpacing(e.target.value);localStorage.setItem('flow_spacing',e.target.value)}}/></Field></div>
