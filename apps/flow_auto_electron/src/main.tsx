...
function App(){
  ...
  async function quickMergeProfile(profile:any){setActivity(`Đang ghép nhanh ${profile.name}...`);const r=await api().quickMerge({folder:profile.dir,transition:quickTransition,autoSub:quickAutoSub,subLang:quickSubLang,apiKey:firstKey(),apiModel:geminiApiModel});if(r?.out)setStatusProfiles((prev:any[])=>prev.map(x=>x.id===profile.id?{...x,merged:r.out,mergedUrl:r.url}:x));append(r)}
  async function quickMergeAll(){for(const profile of statusProfiles)await quickMergeProfile(profile)}

  useEffect(()=>{
    if(page!=='status')return;
    let active=true;
    const refresh=async()=>{try{const r=await api().outputTimeline([danceDownloadDir,downloadDir]);if(active&&r)setOutputTimeline(r);if(statusProfiles.length){const rows=await Promise.all(statusProfiles.map(async p=>({...p,timeline:await api().outputTimeline([p.dir])})));if(active)setStatusProfiles(rows)}}catch{}};
