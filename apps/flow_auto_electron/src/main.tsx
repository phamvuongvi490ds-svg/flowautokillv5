...
function App(){
  ...
  const [mode,setMode]=useState(localStorage.getItem('flow_mode')||'createvideo');
  const [subMode,setSubMode]=useState('frames');
  const [refMatchMode,setRefMatchMode]=useState(localStorage.getItem('flow_ref_match_mode')||'paired');
  ...
  async function aiPostPlan(){append('AI đang tạo timeline + subtitle...'); const r=await api().videoPostPlan({folder:videoFolder,files:videoFiles,script:postScript,subLang,apiKey:firstKey(),apiModel:geminiApiModel}); if(r?.scenes)setTimeline(r.scenes); if(r?.subtitles)setManualSubs(r.subtitles.map((x:any)=>`${x.start}, ${x.end}, ${x.text}`).join('\n')); append(r)}
  ...
  function runPayload(file?:string, overrides:any={}){
    ...
    const payload={
      ...
      referenceMode:refMatchMode, refMatchMode
    };
    ...
    <main className="main">
      ...
      {page==='ai'&&<div className="grid ai">
        <Card title="API & Kịch bản" icon={<Wand2/>}>
          ...
          <div className="form4">
            <Field label="Style"><select value={style} onChange={e=>setStyle(e.target.value)}>{styles.map(x=><option key={x} value={x}>{x==='CINEMATIC'?'LIVE ACTION - người thật':x}</option>)}</select></Field>
            <Field label="Loại"><select value={mediaType} onChange={e=>setMediaType(e.target.value)}><option value="IMAGE">IMAGE</option><option value="VIDEO">VIDEO</option></select></Field><Field label="Ngôn ngữ prompt"><select value={promptLang} onChange={e=>setPromptLang(e.target.value)}><option value="vi">Tiếng Việt</option><option value="en">Tiếng Anh</option><option value="zh">Tiếng Trung</option><option value="ko">Tiếng Hàn</option><option value="es">Tiếng Tây Ban Nha</option></select></Field><Field label="Ngôn ngữ giọng nói nhân vật"><select value={voiceLang} onChange={e=>setVoiceLang(e.target.value)}><option value="vi">Nhân vật nói tiếng Việt</option><option value="en">Nhân vật nói tiếng Anh</option></select></Field><Field label="Model API Gemini"><select value={geminiApiModel} onChange={e=>setGeminiApiModel(e.target.value)}>{geminiApiModels.map(x=><option key={x.value} value={x.value}>{x.label}</option>)}</select></Field>
        ...
        <Card title="Thiết lập Flow" icon={<Film/>}>
          <div className="form4">
            ...
            <Field label="Phương thức tham chiếu"><select value={refMatchMode} onChange={e=>{setRefMatchMode(e.target.value);localStorage.setItem('flow_ref_match_mode',e.target.value)}}><option value="paired">Từng ảnh/prompt (1.jpg → prompt 1)</option><option value="character">Nhân vật/prompt (Mai.jpg → prompt có Mai)</option></select></Field>
      ...
      {page==='multi'&&<div className="multi-page">
        <Card title="Đa luồng profile Flow" icon={<Film/>}>
          ...
          <div className="form4"><Field label="Số profile"><select value={String(profiles.length)} onChange={e=>{const n=Number(e.target.value); setProfiles(p=>Array.from({length:n},(_,i)=>p[i]||{name:`Profile ${i+1}`,accountEmail:'',script:'',promptFile:'',refsDir:''}))}}>{Array.from({length:100},(_,i)=>String(i+1)).map(x=><option key={x} value={x}>{x} profile</option>)}</select></Field><Field label="Chế độ"><select id="flow-mode-multi" value={mode} onChange={e=>changeMode(e.target.value)}><option value="createimage">Tạo ảnh</option><option value="createvideo">Tạo video</option></select></Field><Field label="Phương thức tham chiếu"><select value={refMatchMode} onChange={e=>{setRefMatchMode(e.target.value);localStorage.setItem('flow_ref_match_mode',e.target.value)}}><option value="paired">Từng ảnh/prompt</option><option value="character">Nhân vật/prompt</option></select></Field><Field label="Model"><select id="flow-model-multi" value={model} onChange={e=>{setModel(e.target.value);localStorage.setItem('flow_model',e.target.value)}}>{availableModels.map(x=><option key={x} value={x}>{x}</option>)}</select></Field>{model==='omni_flash'&&<Field label="Thời lượng Omni Flash"><select id="flow-omni-duration" value={omniDuration} onChange={e=>setOmniDuration(e.target.value)}>{['4s','6s','8s','10s'].map(x=><option key={x} value={x}>{x}</option>)}</select></Field>}<Field label="Tỉ lệ"><select id="flow-ratio-multi" value={ratio} onChange={e=>{setRatio(e.target.value);localStorage.setItem('flow_ratio',e.target.value)}}>{ratios.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Số output"><select id="flow-count-multi" value={count} onChange={e=>{setCount(e.target.value);localStorage.setItem('flow_count',e.target.value)}}>{['1','2','3','4'].map(x=><option key={x} value={x}>{x}x</option>)}</select></Field><Field label="Chế độ chạy"><select id="flow-run-mode-multi" value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục</option><option value="continuous_download_delay_3">Chạy liên tục - tải trễ 3 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>{setSpacing(e.target.value);localStorage.setItem('flow_spacing',e.target.value)}}/></Field></div>
