...
import { Bot, Film, KeyRound, Play, Square, Wand2, ImagePlus, CreditCard, Scissors, Music, Sun, Moon } from 'lucide-react';
import './style.css';
...
const api = () => window.flowAPI || {
  ...
  videoList: async()=>({ok:true,files:[]}), videoMerge: async()=>({ok:false,error:'flowAPI_not_ready'}), videoExtractAudio: async()=>({ok:false,error:'flowAPI_not_ready'}), videoAnalyzeSample: async()=>({ok:false,error:'flowAPI_not_ready'}), videoPostPlan: async()=>({ok:false,error:'flowAPI_not_ready'}), videoPostExport: async()=>({ok:false,error:'flowAPI_not_ready'}), danceGenerate: async()=>({ok:false,error:'flowAPI_not_ready'}), danceExtractAudio: async()=>({ok:false,error:'flowAPI_not_ready'})
};
...
function App(){
  ...
  const [hiddenBrowser,setHiddenBrowser]=useState((localStorage.getItem('flow_hidden_browser')||'false')==='true');
  ...
  function saveFlowSettings(){localStorage.setItem('flow_hidden_browser',String(hiddenBrowser)); const safeModel=normalizeModelForMode(model,mode); if(safeModel!==model)setModel(safeModel); localStorage.setItem('flow_mode',mode); localStorage.setItem('flow_model',safeModel); localStorage.setItem('flow_ratio',ratio); localStorage.setItem('flow_count',count); localStorage.setItem('flow_spacing',spacing); localStorage.setItem('flow_run_mode',runMode); localStorage.setItem('flow_auto_download',String(autoDownload)); localStorage.setItem('flow_download_dir',downloadDir); append('✅ Đã lưu cấu hình setting. Lần sau mở app sẽ giữ nguyên tùy chọn.'); }
  const nav=[['status',T('Trạng thái chạy','Run Status'),Play],['flow',T('Vận hành Flow','Flow Operation'),Film],['ai','AI Prompt Studio',Wand2],['chars','Prompt nhân vật',ImagePlus],['multi',T('Đa luồng','Multi-profile'),Film],['post',T('Hậu kì video','Video Post-production'),Scissors],['dance','Dance Prompt Studio',Music],['payment',T('Thanh toán','Payment'),CreditCard],['license','License',KeyRound]];
  ...
  async function generateScript(){
    ...
    try {
      const r=await api().generateScript({apiKey:apiKeys,apiModel:geminiApiModel,style,topic:scriptTopic,duration,characterImages,refsDir,promptLang,voiceLang,speakerGender,promptSubtitles});
      if(r?.ok) {
        if(r.generated?.file) setGeneratedFile(r.generated.file);
  ...
  async function quickMergeProfile(profile:any){if(!profile?.dir){append('❌ Chưa có thư mục video để ghép.');return}setActivity(`⏳ Đang ghép nhanh ${profile.name}...`);const r=await api().quickMerge({folder:profile.dir,transition:quickTransition,autoSub:quickAutoSub,subLang:quickSubLang,apiKey:firstKey(),apiModel:geminiApiModel});if(r?.out){if(profile.id==='default')setOutputTimeline((x:any)=>({...x,merged:r.out,mergedUrl:r.url}));else setStatusProfiles((prev:any[])=>prev.map(x=>x.id===profile.id?{...x,merged:r.out,mergedUrl:r.url}:x))}append(r)}
  ...
  function runPayload(file?:string, overrides:any={}){
    ...
    const payload={
      ...
      hiddenBrowser: overrides.hiddenBrowser ?? hiddenBrowser,
...
<Field label="Chế độ chạy"><select id="flow-run-mode" value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải theo lô 3 prompt</option></select></Field><Field label="Auto tải ảnh/video"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Ảnh và video hoàn tất được tự lưu vào thư mục đã cấu hình; chế độ liên tục tải theo lô 3 prompt.</small></Field><Field label="Chạy trình duyệt ẩn"><label className="switch"><input type="checkbox" checked={hiddenBrowser} onChange={e=>{setHiddenBrowser(e.target.checked);localStorage.setItem('flow_hidden_browser',String(e.target.checked))}}/><span>{hiddenBrowser?'Bật':'Tắt'}</span></label><small className="field-note">Chrome chạy ngoài vùng hiển thị nhưng vẫn có đầy đủ giao diện để Flow hoạt động ổn định.</small></Field><Field label="Số tab Flow"><select id="flow-threads" value={flowThreads} onChange={e=>setFlowThreads(e.target.value)}>{Array.from({length:100},(_,i)=>String(i+1)).map(x=><option key={x} value={x}>{x} tab</option>)}</select></Field></div></Card><Card title="Điều khiển" icon={<Play/>}><div className="actions"><Button variant="primary" onClick={()=>start()}><Play size={16}/> Bắt đầu</Button><Button variant="danger" onClick={stop}><Square size={16}/> Stop</Button></div></Card></div>}
      ...
      {page==='chars'&&<div className="grid ai">
        ...
        <Card title="Thiết lập Flow ảnh" icon={<Film/>}>
          <div className="form4">
<Field label="Mode"><select value={mode} onChange={e=>changeMode(e.target.value)}><option value="createimage">createimage</option><option value="createvideo">createvideo</option></select></Field><Field label="Model"><select value={model} onChange={e=>{setModel(e.target.value);localStorage.setItem('flow_model',e.target.value)}}>{availableModels.map(x=><option key={x} value={x}>{x==='veo3_lite_low_priority'?'Veo 3.1 - Lite [Low Priority]':x}</option>)}</select></Field><Field label="Tỉ lệ"><select value={ratio} onChange={e=>{setRatio(e.target.value);localStorage.setItem('flow_ratio',e.target.value)}}>{ratios.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Số output"><select value={count} onChange={e=>{setCount(e.target.value);localStorage.setItem('flow_count',e.target.value)}}>{['1','2','3','4'].map(x=><option key={x} value={x}>{x}x</option>)}</select></Field><Field label="Giãn cách prompt"><input value={spacing} onChange={e=>{setSpacing(e.target.value);localStorage.setItem('flow_spacing',e.target.value)}} placeholder="10" /></Field><Field label="Chế độ tham chiếu"><select id="flow-ref-mode-multi" value={refMode} onChange={e=>{setRefMode(e.target.value);localStorage.setItem("flow_ref_mode",e.target.value)}}><option value="paired">1 ảnh, 1 prompt</option><option value="all">Tham chiếu toàn bộ ảnh</option></select></Field>
...
<Field label="Mode"><select value={mode} onChange={e=>changeMode(e.target.value)}><option value="createvideo">createvideo</option><option value="createimage">createimage</option></select></Field>
            ...
            <Field label="Model"><select id="flow-model-multi" value={model} onChange={e=>{setModel(e.target.value);localStorage.setItem('flow_model',e.target.value)}}>{availableModels.map(x=><option key={x} value={x}>{x==='veo3_lite_low_priority'?'Veo 3.1 - Lite [Low Priority]':x}</option>)}</select></Field>{model==='omni_flash'&&<Field label="Thời lượng Omni Flash"><select id="flow-omni-duration" value={omniDuration} onChange={e=>setOmniDuration(e.target.value)}>{['4s','6s','8s','10s'].map(x=><option key={x} value={x}>{x}</option>)}</select></Field>}
...
<Field label="Chế độ chạy"><select value={runMode} onChange={e=>{setRunMode(e.target.value);localStorage.setItem('flow_run_mode',e.target.value)}}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục - tải theo lô 3 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>{setSpacing(e.target.value);localStorage.setItem('flow_spacing',e.target.value)}}/></Field>
          ...
          <div className="actions"><Button onClick={saveFlowSettings}>💾 Lưu cấu hình setting</Button></div><div className="form4"><Field label="Chạy trình duyệt ẩn"><label className="switch"><input type="checkbox" checked={hiddenBrowser} onChange={e=>{setHiddenBrowser(e.target.checked);localStorage.setItem('flow_hidden_browser',String(e.target.checked))}}/><span>{hiddenBrowser?'Bật':'Tắt'}</span></label></Field><Field label="Auto tải ảnh/video"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>{setAutoDownload(e.target.checked);localStorage.setItem('flow_auto_download',String(e.target.checked))}}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Ảnh và video hoàn tất được tự lưu vào thư mục đã cấu hình; chế độ liên tục tải theo lô 3 prompt.</small></Field></div><div className="actions"><Button variant="primary" onClick={()=>start(generatedFile,{autoDownload,refsDir,pairedMode:false})}>▶ Chạy prompt AI</Button><Button variant="danger" onClick={stop}>⏹ Stop</Button></div>
        </Card>
      </div>}
