    ...
    <main className="main">
      ...
      {page==='chars'&&<div className="grid ai">
        ...
        <Card title="Thiết lập Flow ảnh" icon={<Film/>}>
          <div className="form4"><Field label="Mode"><select value={mode} onChange={e=>setMode(e.target.value)}><option value="createimage">createimage</option><option value="createvideo">createvideo</option></select></Field><Field label="Model"><select value={model} onChange={e=>setModel(e.target.value)}>{models.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Tỉ lệ"><select value={ratio} onChange={e=>setRatio(e.target.value)}>{ratios.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Số output"><select value={count} onChange={e=>setCount(e.target.value)}>{['1','2','3','4'].map(x=><option key={x} value={x}>{x}x</option>)}</select></Field><Field label="Giãn cách prompt"><input value={spacing} onChange={e=>setSpacing(e.target.value)} placeholder="10" /></Field><Field label="Chế độ chạy"><select value={runMode} onChange={e=>setRunMode(e.target.value)}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục</option><option value="continuous_download_delay_3">Chạy liên tục - tải trễ sau 3 prompt</option></select></Field><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>setAutoDownload(e.target.checked)}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là luôn tự tải, dù chạy từng prompt hay chạy liên tục.</small></Field></div>
          <div className="actions"><Button variant="primary" onClick={()=>start(generatedFile,{mode,model,ratio,count,spacing,runMode,autoDownload})}>▶ Chạy prompt nhân vật</Button><Button variant="danger" onClick={stop}>⏹ Stop</Button></div>
        </Card>
      </div>}
      ...
      {page==='ai'&&<div className="grid ai">
        ...
        <Card title="Thiết lập Flow" icon={<Film/>}>
          <div className="form4">
            ...
            <Field label="Số output"><select id="flow-count" value={count} onChange={e=>setCount(e.target.value)}>{['1','2','3','4'].map(x=><option key={x} value={x}>{x}x</option>)}</select></Field><Field label="Chế độ chạy"><select value={runMode} onChange={e=>setRunMode(e.target.value)}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục</option><option value="continuous_download_delay_3">Chạy liên tục - tải trễ 3 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>setSpacing(e.target.value)}/></Field>
          </div>
          <div className="form4"><Field label="Auto tải xuống"><label className="switch"><input type="checkbox" checked={autoDownload} onChange={e=>setAutoDownload(e.target.checked)}/><span>{autoDownload?'Bật':'Tắt'}</span></label><small className="field-note">Bật là luôn tự tải, dù chạy từng prompt hay chạy liên tục.</small></Field></div><div className="actions"><Button variant="primary" onClick={()=>start(generatedFile,{autoDownload})}>▶ Chạy prompt AI</Button><Button variant="danger" onClick={stop}>⏹ Stop</Button></div>
        </Card>
      </div>}
      {page==='multi'&&<div className="multi-page">
        <Card title="Đa luồng profile Flow" icon={<Film/>}>
          ...
          <div className="form4"><Field label="Số profile"><select value={String(profiles.length)} onChange={e=>{const n=Number(e.target.value); setProfiles(p=>Array.from({length:n},(_,i)=>p[i]||{name:`Profile ${i+1}`,accountEmail:'',script:'',promptFile:'',refsDir:''}))}}>{Array.from({length:100},(_,i)=>String(i+1)).map(x=><option key={x} value={x}>{x} profile</option>)}</select></Field><Field label="Chế độ"><select id="flow-mode-multi" value={mode} onChange={e=>setMode(e.target.value)}><option value="createimage">Tạo ảnh</option><option value="createvideo">Tạo video</option></select></Field><Field label="Model"><select id="flow-model-multi" value={model} onChange={e=>setModel(e.target.value)}>{models.map(x=><option key={x} value={x}>{x}</option>)}</select></Field>{model==='omni_flash'&&<Field label="Thời lượng Omni Flash"><select id="flow-omni-duration" value={omniDuration} onChange={e=>setOmniDuration(e.target.value)}>{['4s','6s','8s','10s'].map(x=><option key={x} value={x}>{x}</option>)}</select></Field>}<Field label="Tỉ lệ"><select id="flow-ratio-multi" value={ratio} onChange={e=>setRatio(e.target.value)}>{ratios.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Số output"><select id="flow-count-multi" value={count} onChange={e=>setCount(e.target.value)}>{['1','2','3','4'].map(x=><option key={x} value={x}>{x}x</option>)}</select></Field><Field label="Chế độ chạy"><select id="flow-run-mode-multi" value={runMode} onChange={e=>setRunMode(e.target.value)}><option value="single">Chạy từng prompt một</option><option value="continuous_submit_only">Chạy liên tục</option><option value="continuous_download_delay_3">Chạy liên tục - tải trễ 3 prompt</option></select></Field><Field label="Giãn cách prompt"><input id="flow-spacing" value={spacing} onChange={e=>setSpacing(e.target.value)}/></Field></div>
